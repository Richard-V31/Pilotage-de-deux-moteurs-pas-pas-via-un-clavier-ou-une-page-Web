...
const char* password = SECRET_PASS;// Récupère le mot de passe WiFi défini dans arduino_secrets.h
...
// =====================
#define VITESSE_MIN 3       // Vitesse minimale autorisée (pas par seconde)
...
// 🟢 Multiplicateur appliqué à la vitesse du mode "1 Pas" quand la case "x5" est cochée (à modifier ICI uniquement)
#define FACTEUR_1PAS 5  // Variable a modifier pour le Mode 1Pas+
...
// 🟢 Etat courant du "Pas à Pas" (vrai que l'appui vienne du clavier physique ou du web), pour affichage sur la page web
volatile bool step1Actif = false;  // true tant que M1 est en pas à pas (bouton physique 1 ou bouton web maintenu)
volatile bool step2Actif = false;  // idem pour M2

// 🟢 Case à cocher web "x5" : force la vitesse du mode "1 Pas" (manuel) à x5, indépendamment pour chaque moteur
volatile bool pas5M1 = false;   // true = le mode "1 Pas" du moteur 1 tourne à 5x la vitesse manuelle normale
volatile bool pas5M2 = false;   // true = le mode "1 Pas" du moteur 2 tourne à 5x la vitesse manuelle normale
...
<style>
  ...
  /* 🟢 Ligne regroupant le bouton "1 Pas" (réduit) et la case à cocher "x5" à côté */
  .step-row {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .step-row .step-btn {
    flex: 1;              /* Le bouton "1 Pas" occupe le reste de la place, réduit par la case x5 */
    margin: 0;
  }
  /* Bloc "switch x5" (case à cocher stylée) + son étiquette, à droite du bouton "1 Pas" */
  .pas5-bloc {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  .pas5-bloc span {
    font-size: 1em;  /* Taille du texte x5 */
    color: #fff;
  }
  /* Case à cocher classique (native) pour le mode "1 Pas" x)HTML" STR(FACTEUR_1PAS) R"HTML( */
  .checkbox-label {
    display: flex;
    flex-direction: column;
    align-items: center;
...
<div class="moteur">
  ...
  <!-- 🟢 Réduit (flex:1 via .step-row) pour laisser la place à la case à cocher "x5" -->
  <div class="step-row">
    <button id="m1step" class="step-btn" title="Pas à Pas (maintenir)"
      ontouchstart="stepStart('m1')" ontouchend="stepStop('m1')"
      onmousedown="stepStart('m1')" onmouseup="stepStop('m1')" onmouseleave="stepStop('m1')">
      Pas à Pas
    </button>
    <!-- 🟢 Case à cocher "xFACTEUR_1PAS" : force la vitesse du mode "1 Pas" pour le Moteur 1 -->
    <!-- L'étiquette est générée depuis la macro FACTEUR_1PAS : toujours cohérente avec la vitesse réellement appliquée -->
    <div class="pas5-bloc">
      <label class="checkbox-label">
        <input type="checkbox" id="m1pas5" onchange="pas5Change('m1', this.checked)">
        <span>x)HTML" STR(FACTEUR_1PAS) R"HTML(</span>
      </label>
    </div>
  </div>
...
<div class="moteur">
  ...
  <div class="step-row">
    <button id="m2step" class="step-btn" title="Pas à Pas (maintenir)"
      ontouchstart="stepStart('m2')" ontouchend="stepStop('m2')"
      onmousedown="stepStart('m2')" onmouseup="stepStop('m2')" onmouseleave="stepStop('m2')">
      Pas à Pas
    </button>
    <!-- 🟢 Case à cocher "xFACTEUR_1PAS" : force la vitesse du mode "1 Pas" pour le Moteur 2 -->
    <!-- L'étiquette est générée depuis la macro FACTEUR_1PAS : toujours cohérente avec la vitesse réellement appliquée -->
    <div class="pas5-bloc">
      <label class="checkbox-label">
        <input type="checkbox" id="m2pas5" onchange="pas5Change('m2', this.checked)">
        <span>x)HTML" STR(FACTEUR_1PAS) R"HTML(</span>
      </label>
    </div>
  </div>
...
// 🟢 Appelée quand on coche/décoche la case "x5" (force la vitesse du mode "1 Pas" à x5)
function pas5Change(m, checked){
  fetch("/"+m+"/pas5?val=" + (checked ? "1" : "0"));   // Informe l'Arduino du nouvel état pour ce moteur
}
...
function refresh(){
  fetch("/status").then(r=>r.json()).then(s=>{
    ...
    // 🟢 Colore aussi le bouton "Pas à Pas" en bleu quand c'est le clavier physique qui est utilisé
    document.getElementById("m1step").classList.toggle("active", s.step1);
    document.getElementById("m2step").classList.toggle("active", s.step2);
    // 🟢 Resynchronise les cases "x5" à chaque rafraîchissement (simple case à cocher, pas de risque de "conflit de glissement" comme les sliders)
    document.getElementById("m1pas5").checked = s.pas5M1;
    document.getElementById("m2pas5").checked = s.pas5M2;
    if (premierRefresh) {
...
setInterval(refresh, 500);  // 📌🚨Rafraîchit l'état des boutons affiché toutes les 500 ms entre 300 et 700
...
client.setTimeout(5); // 👈 Réduit le temps d'attente maximum à 5ms pour libérer le serveur plus vite
  ...
  } else if (path == "/m1/pas5") {
    pas5M1 = (valParam == "1");                         // 🟢 Active/désactive la vitesse x5 du mode "1 Pas" du moteur 1
  } else if (path == "/m2/pas5") {
    pas5M2 = (valParam == "1");                         // 🟢 Active/désactive la vitesse x5 du mode "1 Pas" du moteur 2
  }
  ...
  if (path == "/status") {
    ...
    json += "\"pas5M1\":" + String(pas5M1 ? 1 : 0) + ",";     // 🟢 Etat de la case "x5" du moteur 1
    json += "\"pas5M2\":" + String(pas5M2 ? 1 : 0) + ",";     // 🟢 Etat de la case "x5" du moteur 2
    json += "\"step1\":" + String(step1Actif ? 1 : 0) + ",";  // 🟢 Pas à Pas M1 actif (physique ou web)
    json += "\"step2\":" + String(step2Actif ? 1 : 0);        // 🟢 Pas à Pas M2 actif (physique ou web)
    json += "}";
...
void loop() {
  ...
  step1Actif = step1;   // 🟢 Recopie dans la variable globale, lue par traiterClientWeb() pour le JSON /status
  step2Actif = step2;   // 🟢 idem pour M2
  ...
  if (step1) {
    ...
    float vitesseManuelleM1 = pas5M1 ? (vitesseManuelle * FACTEUR_1PAS) : vitesseManuelle;  // 🟢 xFACTEUR_1PAS si la case est cochée
    float vManM1 = invM1 ? -vitesseManuelleM1 : vitesseManuelleM1;   // Applique le sens (inversé ou non)
  ...
  if (step2) {
    ...
    float vitesseManuelleM2 = pas5M2 ? (vitesseManuelle * FACTEUR_1PAS) : vitesseManuelle;  // 🟢 xFACTEUR_1PAS si la case est cochée
    float vManM2 = invM2 ? -vitesseManuelleM2 : vitesseManuelleM2;
